...
const nightModeControl = document.querySelector('.night-mode-control');
nightModeControl.addEventListener('click', (e) => {
    if (e.target.classList.contains('night-mode-control')) {
        document.querySelector('body').classList.toggle('night-mode');
    }
});

class Chart {
    constructor({ data, parent, isAutoRender = true, width = 300, height = 300 }) {
        if(!parent) {
            throw new Error('parent param is required');
        }

        this.canvas = document.createElement('canvas');
        this.canvas.classList.add('telegram-charts__canvas');
        this.canvas.setAttribute('width', width);
        this.canvas.setAttribute('height', height);
        
        document.querySelector(parent).appendChild(this.canvas);
    ...
    render() {
        ...
        this._drawYGuidlines({canvasCtx});
     ...
     * Draw Y guidelines
     */
    _drawYGuidlines({canvasCtx}) {
        const { canvasHeight, canvasWidth } = this;
        const linesCount = 6;
        const step = Math.round(this.canvasHeight / linesCount);
        let yPosition = 0;
        ...
        for (let i = 0; i < linesCount; i++) {
            ...
            canvasCtx.font = '11px serif';
            canvasCtx.fillStyle = '#c3c3c3';
            canvasCtx.fillText(Math.abs(yPosition - 300), 4, yPosition - 4);
    ...
    _prepareData(data) {
        ...
        data.columns.forEach(column => {
            const columnMin = Math.min.apply(this, column.slice(1, column.length));
            const columnMax = Math.max.apply(this, column.slice(1, column.length));
            ...
            column.forEach((item, index) => {
                if (index === 0) {
                    ...
                    if (column[0] === 'x') {
                        ...
                        preparedData.lines[column[0]].push(scaledValue);
                    }
                }
            });  
        });
        ...
        preparedData.names = data.names;

        return preparedData;
    }

    _scale({ value, oldMin, oldMax, newMin, newMax}) {
    ...
    _createControls() {
        ...
        for (let key in this.state.controls) {
            ...
            control.style.left = '-100000px';

            const icon = document.createElement('div');
            icon.classList.add('controls__icon');
            icon.style.background = this.data.colors[key];

            controlItem.classList.add('controls__item');
            label.style.pointerEvents = 'none';
            label.innerText = this.data.names[key];
            ...
            controlItem.addEventListener('click', this._toggleControlItem.bind(this, { control, icon, lineKey: key }));
     ...
     * Toogle contol item
     */
    _toggleControlItem({ control, icon, lineKey }) {
        control.checked = !control.checked;
        control.dispatchEvent(new Event('change'));

        if (control.checked) {
            icon.style.background = this.data.colors[lineKey];
        } else {
     ...
     */
    _createXAxisLabels() {
        const xLabels = document.getElementById(`${this.id}`).querySelector('.label-x-wrapper');
        let labelsWrapper;

        if (xLabels) {
            xLabels.querySelector('.label-x-content').remove();
            labelsWrapper = xLabels;
        } else {
            labelsWrapper =  document.createElement('div');
            labelsWrapper.classList.add('label-x-wrapper');
        }

        let xPosition = 0;
        let prevLabel;
        const labelsContent = document.createElement('div');
        labelsContent.classList.add('label-x-content');

        labelsWrapper.appendChild(labelsContent);

        if (!xLabels) {
            this.wrapper.appendChild(labelsWrapper);
        }
        ...
        this.data.lines.x.slice(this.sliceLeft, this.sliceRight).forEach((dateString) => {
            ...
            labelsContent.appendChild(label);
...
chartsData.forEach((chartData) => {
    new Chart({ data: chartData, parent: '.telegram-charts' });
});
